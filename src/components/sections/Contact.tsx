...
import { Send, Mail, MapPin, Globe, Terminal, Radio, Briefcase, Github } from 'lucide-react';
import { useForm, ValidationError } from '@formspree/react';

export default function Contact() {
  const [state, handleSubmit] = useForm('mgodgrka');
    ...
    <section id="contact" className="py-32 px-6 relative overflow-hidden bg-brand-deep/20">
      ...
      <div className="app-container relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center mb-32">
          ...
          {/* Right Side: Working Form with Formspree */}
            ...
            <div className="glass p-10 md:p-14 rounded-[4rem] border border-white/5 relative bg-brand-deep shadow-black shadow-2xl overflow-hidden group hover:border-cyan-glow/20 transition-all duration-700">
              ...
              {state.succeeded ? (
                <div className="text-center py-12">
                  <div className="text-cyan-glow text-6xl mb-4">✓</div>
                  <h3 className="text-2xl font-display text-white mb-2">Message received</h3>
                  <p className="text-white/50 font-mono text-sm">Transmission complete – we'll be in touch soon.</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label htmlFor="name" className="text-[10px] font-mono text-white/30 uppercase tracking-[0.2em] ml-1 font-bold">
                        Identity
                      </label>
                      <input
                        id="name"
                      ...
                      />
                      <ValidationError field="name" errors={state.errors} className="text-red-400 text-xs font-mono mt-1" />
                    </div>
                    <div className="space-y-2">
                      <label htmlFor="email" className="text-[10px] font-mono text-white/30 uppercase tracking-[0.2em] ml-1 font-bold">
                        Email
                      </label>
                      <input
                        id="email"
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-white focus:outline-none focus:border-cyan-glow/30 focus:bg-white/10 transition-all font-mono text-sm placeholder:opacity-20"
                      />
                      <ValidationError field="email" errors={state.errors} className="text-red-400 text-xs font-mono mt-1" />
                    </div>
                  </div>

                  <div className="space-y-2">
                    <label htmlFor="message" className="text-[10px] font-mono text-white/30 uppercase tracking-[0.2em] ml-1 font-bold">
                      Message
                    </label>
                    <textarea
                      id="message"
                    ...
                    />
                    <ValidationError field="message" errors={state.errors} className="text-red-400 text-xs font-mono mt-1" />
                  </div>

                  <button
                    type="submit"
                    disabled={state.submitting}
                    className="w-full interactive group relative py-6 bg-cyan-glow text-brand-deep rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] overflow-hidden transition-all hover:scale-[1.02] active:scale-98 shadow-[0_20px_60px_-10px_rgba(6,182,212,0.4)] disabled:opacity-50 disabled:cursor-not-allowed"
                  >
